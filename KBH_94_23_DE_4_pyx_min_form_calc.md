...
\#Problems going forward make sure only one crystal info is being
uploaded at a time. Complications abound with end math cannoth sum and
average chunk had to do manually. You did not have fun that day.

``` r
library(readr)
KBH_94_23_DE_4_R <- read_csv("KBH_94_23_DE_4_R.csv")
```
...
all points in this set are CPX from 2 diff crystals

sum\_row &lt;- sum\_ox %&gt;% mutate(sum = rowSums(.))

``` r
sum_cols <- mult_cations %>% 
  select(a1,
         a2,
         a3,
         a4,
